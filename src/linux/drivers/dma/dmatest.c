...
static bool ext_width;
module_param(ext_width, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(ext_width,
		"extend pattern width for memset 0-8bit 1-32bit (default: 0)");
...
static int dmatest_func(void *data)
{
	...
	while (!kthread_should_stop()
		...
		if (thread->type == DMA_MEMCPY)
			...
			uint8_t pattern = *(thread->srcs[0] + src_off);
			uint32_t ext_pattern = ((pattern << 24) |
					       (pattern << 16) |
					       (pattern << 8) | pattern);
			int val = (int) (ext_width) ? ext_pattern : pattern;
