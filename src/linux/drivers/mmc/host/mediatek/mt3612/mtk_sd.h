...
#define REG_ADDR_TOP(x)	((u32 *)(base_top + OFFSET_##x))
...
struct msdc_host {
	...
	void __iomem            *base_top;	/* base address for msdc top */
	int                     id;             /* host id */
	...
	struct clk              *hclk_control;
	struct clk              *aes_clk_control;
...
#define MSDC_WRITE8(reg, val) \
	do {    \
		__raw_writeb((val), (void __force __iomem *)((reg)));   \
		mb();  /* fix for memory barrier without comment */ \
	} while (0)
#define MSDC_WRITE16(reg, val) \
	do {    \
		__raw_writew((val), (void __force __iomem *)((reg)));   \
		mb();  /* fix for memory barrier without comment */ \
	} while (0)
#define MSDC_WRITE32(reg, val)  \
	do {    \
		__raw_writel((val), (void __force __iomem *)((reg)));   \
		mb();  /* fix for memory barrier without comment */ \
	} while (0)
