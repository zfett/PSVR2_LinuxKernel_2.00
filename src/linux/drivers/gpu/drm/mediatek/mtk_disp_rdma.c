...
{
	cmdq_pkt_write_mask(handle, RDMA_ENGINE_EN, comp->cmdq_base,
			    ...
			    unsigned int bpc, struct cmdq_pkt *handle)
{
	...
	cmdq_pkt_write_mask(handle, width, comp->cmdq_base,
			    DISP_REG_RDMA_SIZE_CON_0, 0x1fff);
	cmdq_pkt_write_mask(handle, height, comp->cmdq_base,
	...
	cmdq_pkt_write(handle, reg, comp->cmdq_base, DISP_REG_RDMA_FIFO_CON);
}
