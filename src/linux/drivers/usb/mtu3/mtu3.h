...
/** @ingroup IP_group_usb_internal_def
 * @{
 */
...
/** @ingroup IP_group_usb_internal_enum
* @brief device operated link and speed got from DEVICE_CONF register\n
* value is from 0 to 5.
*/
enum mtu3_speed {
/** 0: USB SPEED INACTIVE */
	MTU3_SPEED_INACTIVE = 0,
/** 1: USB FULL SPEED */
	MTU3_SPEED_FULL = 1,
/** 3: USB RG Read is HIGH SPEED */
	MTU3_SPEED_HIGH = 3,
/** 4: USB RG Read is SUPER SPEED */
	MTU3_SPEED_SUPER = 4,
/** 5: USB RG Read is SUPER SPEED PLUS */
 ...
 */
/** @ingroup IP_group_usb_internal_struct
 * @brief General Purpose Descriptor Structure.
 */
...
struct qmu_gpd {
	...
	__le16 data_buf_len;
	/** Physical address of the next GPD */
	__le32 next_gpd;
...
struct mtu3_gpd_ring {
	/** physical base address of GPD segment */
	dma_addr_t dma;
	/** virtual base address of GPD segment */
	struct qmu_gpd *start;
	/** the last GPD element */
	struct qmu_gpd *end;
	/** enqueue: the first empty GPD to use */
...
struct otg_switch_mtk {
	...
	/** extcon_reg_dwork: delay work for extcon notifier register */
	struct delayed_work extcon_reg_dwork;
	/** is_u3_drd: port0 supports usb3.0 dual-role device or not */
	bool is_u3_drd;
	/** dual-role switch by debugfs */
	struct pinctrl *id_pinctrl;
	struct pinctrl_state *id_float;
	struct pinctrl_state *id_ground;
	/** Tt's true when supports dual-role device by debugfs */
...
struct ssusb_mtk {
	struct device *dev;
	struct mtu3 *u3d;
	/** register base address of device MAC, exclude xHCI's */
	void __iomem *mac_base;
	/** register base address of IP Power and Clock interface (IPPC) */
	void __iomem *ippc_base;
	struct phy **phys;
	int num_phys;
	/** usb3.3V shared by device/host IP */
	struct regulator *vusb33;
	/** system clock of mtu3 */
	struct clk *sys_clk;
	/** reference clock of mtu3 */
	struct clk *ref_clk;
	struct clk *mcu_clk;
	struct clk *dma_clk;
	/* otg */
	...
	bool is_host;
	/** number of usb2.0 host ports */
	int u2_ports;
	/** number of usb3.0 host ports */
	int u3_ports;
	/** only used when supports manual dual-role switch via debugfs */
	struct dentry *dbgfs_root;
	/** usb wakeup for host mode */
	bool wakeup_en;
	/** port0's wakeup debounce clock */
	struct clk *wk_deb_p0;
	/** it's optional, and depends on port1 is supported or not */
