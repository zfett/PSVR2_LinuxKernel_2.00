...
static void mmc_post_req(struct mmc_host *host,
	...
	struct mmc_async_req *areq_que)
{
	...
	areq_que->mrq->cmd->data = areq_que->mrq->data;
	areq_que->mrq->data->error = 0;
	areq_que->mrq->data->mrq = areq_que->mrq;
	if (areq_que->mrq->stop) {
		areq_que->mrq->data->stop = areq_que->mrq->stop;
...
int mmc_run_queue_thread(void *data)
{
	...
	while (1) {
		...
		if (!atomic_read(&host->cq_rw)) {
			...
			if (dat_mrq) {
				WARN_ON(dat_mrq->cmd->opcode !=
					MMC_WRITE_REQUESTED_QUEUE
					&& dat_mrq->cmd->opcode !=
						MMC_READ_REQUESTED_QUEUE);
		...
		if (is_done) {
			...
			mmc_blk_end_queued_req(host, done_mrq->areq,
				task_id, err);
			/* FIX ME*/
		...
		do {
			if ((resp & 1) && (!host->data_mrq_queued[i])) {
				if (host->cur_rw_task == i) {
					pr_err("task %d rdy not clr when DMA\n",
						i);
					resp >>= 1;
					i++;
					continue;
