...
struct met_api_tbl {
	...
	int (*met_tag_async_start)(unsigned int class_id, const char *name,
				    unsigned int cookie);
	int (*met_tag_async_end)(unsigned int class_id, const char *name,
				  unsigned int cookie);
	int (*met_tag_oneshot)(unsigned int class_id, const char *name,
				unsigned int value);
	int (*met_tag_userdata)(char *pData);
	int (*met_tag_dump)(unsigned int class_id, const char *name,
	...
	int (*met_show_clk_tree)(const char *name, unsigned int addr,
				  unsigned int status);
...
MET_DEFINE_PROBE(sched_switch, TP_PROTO(bool preempt, struct task_struct *prev,
					struct task_struct *next))
{
...
void met_mmsys_event_gce_thread_end(ulong thread_no, ulong task_handle,
				    ulong engineFlag)
{
}
