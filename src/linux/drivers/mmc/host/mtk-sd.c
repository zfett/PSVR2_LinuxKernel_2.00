...
#define MSDC_AES_CTR0_GP0        (0x620)
#define MSDC_AES_CTR1_GP0        (0x624)
#define MSDC_AES_CTR2_GP0        (0x628)
#define MSDC_AES_CTR3_GP0        (0x62C)

#define MSDC_AES_SWST	0x670
#define AES_SWITCH_START_ENC                    (0x1<<0)
#define AES_SWITCH_START_DEC                    (0x1<<1)
#define AES_BYPASS                      (0x1<<2)

#define MSDC_AES_CFG_GP1 0x674
#define AES_DATA_UNIT_SIZE_512_BYTE             (0x200<<16)     /* RW */
#define AES_KEY_SIZE_128_BIT                    (0<<8)          /* RW */
#define AES_MODE_XTS 0x04
...
struct msdc_host {
	...
	struct clk *aes_clk_cg; /* msdc aes clock control gate */
...
static const struct mtk_mmc_compatible mt3612_compat = {
	.clk_div_bits = 12,
	.hs400_tune = false,
	.pad_tune_reg = MSDC_PAD_TUNE0,
	.async_fifo = true,
...
static const struct of_device_id msdc_of_ids[] = {
	...
	{ .compatible = "mediatek,mt3612-mmc", .data = &mt3612_compat},
	{}
};
...
{
#ifdef MSDC_XTS_ENABLE
	writel(AES_DATA_UNIT_SIZE_512_BYTE |
	       AES_KEY_SIZE_128_BIT | AES_MODE_XTS,
	       host->base + MSDC_AES_CFG_GP1);
	aes_switch_on = 1;
#endif
}
...
void msdc_xts_disable(struct msdc_host *host)
{
	...
	sdr_set_field(host->base + MSDC_AES_SWST, AES_BYPASS, 1);
#endif
}
...
static void aes_engine_ctrl(struct msdc_host *host, uint32_t opcode)
{
	...
	if (opcode == MMC_WRITE_MULTIPLE_BLOCK ||
		...
		if (aes_switch_on)
			sdr_set_field(host->base + MSDC_AES_SWST, AES_BYPASS,
				      0);
	}

	if ((readl(host->base + MSDC_AES_EN) & AES_EN) &&
	   !(readl(host->base + MSDC_AES_SWST) & AES_BYPASS)) {
		aes_sw_bak = readl(host->base + MSDC_AES_SWST);
		aes_sw_bak &= 0xFFFFFFFC;
		if (opcode == MMC_WRITE_MULTIPLE_BLOCK ||
		    opcode == MMC_WRITE_BLOCK) {
			writel(aes_sw_bak | AES_SWITCH_START_ENC,
			       host->base + MSDC_AES_SWST);
			...
			if (!(aes_sw_bak & AES_BYPASS))
			   ...
			   opcode == MMC_READ_SINGLE_BLOCK) {
			writel(aes_sw_bak | AES_SWITCH_START_DEC,
				host->base + MSDC_AES_SWST);
...
{
	clk_disable_unprepare(host->aes_clk_cg);
...
static void msdc_ungate_clock(struct msdc_host *host)
{
	...
	clk_prepare_enable(host->aes_clk_cg);
...
#ifdef MSDC_XTS_ENABLE
	writel(cmd->arg, host->base + MSDC_AES_CTR0_GP0);
	writel(0, host->base + MSDC_AES_CTR1_GP0);
	writel(0, host->base + MSDC_AES_CTR2_GP0);
	writel(0, host->base + MSDC_AES_CTR3_GP0);
	writel(cmd->arg, host->base + MSDC_AES_CTR0_GP1);
	writel(0, host->base + MSDC_AES_CTR1_GP1);
	writel(0, host->base + MSDC_AES_CTR2_GP1);
	writel(0, host->base + MSDC_AES_CTR3_GP1);
#endif

#ifdef MSDC_XTS_ENABLE
	msdc_xts_disable(host);
#endif
	aes_engine_ctrl(host, cmd->opcode);
		...
		if (host->dev_comp->enhance_rx) {
			if (host->top_base)
				...
				sdr_set_bits(host->base + SDC_ADV_CFG0,
					     SDC_RX_ENHANCE_EN);
			/* MT3612's enhance rx is at 0x64[20] */
			sdr_set_bits(host->base + SDC_ADV_CFG0,
				     SDC_RX_ENHANCE_EN);
		} else {
			sdr_set_field(host->base + MSDC_PATCH_BIT2,
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	host->aes_clk_cg = devm_clk_get(&pdev->dev, "aes_cg");
	if (IS_ERR(host->aes_clk_cg))
		host->aes_clk_cg = NULL;
